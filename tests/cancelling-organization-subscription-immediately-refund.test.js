...
// immediately with a refund for the unused balance
// 
...
// 6) the app is uninstalled and a refund is created

describe(`tests/cancelling-organization-subscription-refund`, () => {
  it('should refund organization subscription and uninstall for all users', async () => {
    global.pageSize = 40
    // owner
    ...
    await TestHelper.completeForm(customer1Tab, {})
    // customer 1 has uninstalled the app and received a refund
    await TestHelper.clickPageLink(customer1Tab, 'Home')
    await customer1Tab.waitFor(1000)
    ...
    assert.strictEqual(appLink1Exists, -1)
    ...
    assert.strictEqual(appLink2Exists > -1, true)
    await TestHelper.clickPageLink(customer1Tab, 'Home')
    await TestHelper.clickPageLink(customer1Tab, 'Credits')
    const creditsFrame = await TestHelper.getApplicationFrame(customer1Tab)
    const creditExists = await creditsFrame.evaluate(() => {
      return document.getElementById('credits-table').rows.length
    })
    assert.strictEqual(creditExists, 2)
    // customer 2 has uninstalled the app
    await TestHelper.clickPageLink(customer2Tab, 'Home')
    await customer1Tab.waitFor(1000)
    ...
    assert.strictEqual(appLink3Exists, -1)
    ...
    assert.strictEqual(appLink4Exists > -1, true)
    browser1.close()
    browser2.close()
    browser3.close()
    browser4.close()
